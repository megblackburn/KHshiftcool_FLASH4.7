...
#include "constants.h"
  integer :: kk 
  double precision :: densMix, tcoolCloud,tcoolWind,tcoolMix
  double precision :: LambdaCloud, LambdaWind, LambdaMix
  logical :: threadBlockListBuild, threadWithinBlockBuild  
  real    :: vctr
  double precision :: cl_kB
  
  call PhysicalConstants_get("Boltzmann", cl_kB)
 
  call RuntimeParameters_get('sim_shift_start', sim_shift_start)
  call RuntimeParameters_get('sim_amb_rho', sim_amb_rho)
  call RuntimeParameters_get('sim_velx', sim_velx)
  call RuntimeParameters_get('sim_rhoHot', sim_rhoHot)
  call RuntimeParameters_get('sim_rhoCold', sim_rhoCold)
  call RuntimeParameters_get('sim_tempHot', sim_tempHot)
  call RuntimeParameters_get('sim_tempCold', sim_tempCold)
...
!!$  print*,'sim_rhoAmbient is',sim_rhoAmbient
  ...
  call RuntimeParameters_get('sim_cloudTemp', sim_cloudTemp)
  call RuntimeParameters_get('sim_cloudDens',  sim_cloudDens)
  
  call RuntimeParameters_get('sim_densContrast', sim_densContrast)
  sim_tempWind = sim_cloudTemp * sim_densContrast
  sim_densWind = sim_cloudDens / sim_densContrast
  densMix = sqrt(sim_cloudDens * sim_densWind)
  sim_tempMix = sqrt(sim_cloudTemp * sim_tempWind)
  sim_coreTemp = sim_cloudTemp / sim_densContrast
  sim_coreDens = sim_cloudDens * sim_densContrast
  sim_kb_mu_mp = cl_kB / (sim_wmu * sim_amu)
  ...
  call compute_Lambda(sim_cloudTemp, kk, LambdaCloud)
  call compute_Lambda( sim_tempWind, kk, LambdaWind )
  call compute_Lambda(  sim_tempMix, kk, LambdaMix  )

  call compute_tcool(sim_cloudDens, sim_cloudTemp, LambdaCloud, tcoolCloud)
  call compute_tcool( sim_densWind,  sim_tempWind, LambdaWind,  tcoolWind)
  call compute_tcool(      densMix,   sim_tempMix, LambdaMix,   tcoolMix)
  sim_presWind = sim_kb_mu_mp * sim_densWind * sim_tempWind
  sim_eintWind = sim_presWind / (sim_densWind * (sim_gamma_gas - 1.0))
