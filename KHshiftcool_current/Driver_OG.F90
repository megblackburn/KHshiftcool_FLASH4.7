    ...
    do k = blkLimitsGC(LOW,KAXIS), blkLimitsGC(HIGH,KAXIS)
     ...
     !!     call Grid_getBlkPtr(blkID, solnData)
         ...
         !! global_vol = sizeX*sizeY*sizeZ
                ...
                front_posn_new = front_posn_new + global_cold_mass/(LX*LY*sim_rhoHot*sim_tempHot/sim_tempCold)
                v_shift_t_new = v_shift_t_new + (-2.0 * (front_posn_new-front_posn_old)/dr_dt) !! use Driver_data, ONLY : dr_dt
