 ...
 !   endif
    ...
    do k = blkLimitsGC(LOW,KAXIS), blkLimitsGC(HIGH,KAXIS)
     ...
     !     if (i>1 .and. j>1) then
         ...
         !   if (dr_globalMe == MASTER_PE) then
       !       write(*,*) "in triple if statement i=", i
        !    endif
  ...
  !          if (dr_globalMe ==MASTER_PE) then
   !           write(*,*) "local_cold_mass", local_cold_mass
    !          write(*,*) "local_v2_sum", local_v2_sum
     !       endif
          call MPI_ALLREDUCE(local_cold_mass, global_cold_mass, 1, FLASH_REAL, MPI_SUM, &
                             dr_globalComm, ierr) !!ierr
          call MPI_ALLREDUCE(local_v2_sum, global_v2_sum, 1, FLASH_REAL, MPI_SUM, &
                             dr_globalComm, ierr)  !!ierr

          vy_avg = global_v2_sum/global_vol
          
      !      if (dr_globalMe == MASTER_PE) then      
       !       write(*,*) "sim_tempCold", sim_tempCold
        !      write(*,*) "sim_tempHot", sim_tempHot
         !     write(*,*) "sim_rhoHot", sim_rhoHot
          !  endif
  ...
  !       energy = solnData(ENER_VAR,i,j,k)
 !        energy_int = solnData(EINT_VAR,i,j,k)
